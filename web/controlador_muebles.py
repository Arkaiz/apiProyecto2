...
def insertar_mueble(nombre, descripcion, precio,foto):
    try:
        conexion = obtener_conexion()
        with conexion.cursor() as cursor:
            cursor.execute("INSERT INTO muebles(nombre, descripcion, precio,foto) VALUES (%s, %s, %s,%s)",
...
def obtener_muebles():
    try:
        conexion = obtener_conexion()
        with conexion.cursor() as cursor:
            cursor.execute("SELECT id, nombre, descripcion, precio,foto FROM muebles")
...
def obtener_mueble_por_id(id):
    ...
    try:
        conexion = obtener_conexion()
        with conexion.cursor() as cursor:
            #cursor.execute("SELECT id, nombre, descripcion, precio,foto FROM muebles WHERE id = %s", (id,))
...
def actualizar_mueble(id, nombre, descripcion, precio, foto):
    try:
        conexion = obtener_conexion()
        with conexion.cursor() as cursor:
            cursor.execute("UPDATE muebles SET nombre = %s, descripcion = %s, precio = %s, foto=%s WHERE id = %s",
